#!/usr/bin/env python3
"""Collect the frozen Topic 005 bounded-subarray experiment and retain every attempt."""
...
SCHEMA_VERSION = 1
ASSIGNMENT_SEED = 0x0000_0005_5A17_D0C5
FAMILY_ALPHA = 0.05
FAMILY_SIZE = 12
...
EXPECTED_LINKED_SYMBOL_SUBSTRINGS = (
    "topic005_reference_workload",
    "topic005_direct_workload",
    "topic005_quadratic_workload",
    "topic005_reset_workload",
    "topic005_prefix_workload",
    "topic005_timed_loop",
    "topic005_consume_result",
    "topic005_clock_now",
)
EXTERNAL_SIGNALS = {
    resolved
...
BENCH_SOURCE = TOPIC_DIR / "benches" / "bounded_subarrays.rs"

CANDIDATES = ("reference", "direct", "quadratic", "reset", "prefix")
BASELINE = "direct"
...
class Cell:
    ...
    expected_count: int
    zero_values: int
    oversized_values: int
    shape: str


CELLS = (
    Cell("n8_zero_heavy_budget0", 8, 0, 6, 5, 3, "four_zero_runs"),
    Cell("n8_all_fit", 8, 13, 36, 2, 0, "all_subarrays_fit"),
    Cell("n64_immediate_reject", 64, 0, 0, 0, 64, "every_singleton_rejected"),
    Cell(
        "n4096_immediate_reject",
        4_096,
        0,
        0,
        0,
        4_096,
        "every_singleton_rejected",
    ),
    Cell("n4096_all_fit", 4_096, 4_096, 8_390_656, 0, 0, "all_subarrays_fit"),
    Cell(
        "n65536_all_fit",
        65_536,
        65_536,
    ...
    Cell(
        "n65536_oversized_every64",
        65_536,
        63,
        2_064_384,
        0,
        1_024,
        "oversized_then_63_ones_repeated",
    ),
    Cell(
        ...
        32_768,
        32_768,
        "oversized_zero_alternation",
    ),
    Cell(
        "n64_zero_heavy_budget0",
        64,
        0,
        96,
        48,
        16,
        "sixteen_zero_runs_of_three",
    ),
    Cell(
        "n4096_uniform_moderate",
        4_096,
        32,
        130_576,
        0,
        0,
        "uniform_ones_window_32",
    ),
)
...
class Contrast:
    ...
    b_candidate: str
    cell: str
    operation_phase: str = "count"


CONTRASTS = (
    Contrast("reference_vs_direct__n8_zero_heavy_budget0", "reference", "n8_zero_heavy_budget0"),
    Contrast("reference_vs_direct__n8_all_fit", "reference", "n8_all_fit"),
    Contrast("quadratic_vs_direct__n8_all_fit", "quadratic", "n8_all_fit"),
    Contrast("quadratic_vs_direct__n64_immediate_reject", "quadratic", "n64_immediate_reject"),
    Contrast(
        "quadratic_vs_direct__n4096_immediate_reject",
        "quadratic",
        "n4096_immediate_reject",
    ),
    Contrast("quadratic_vs_direct__n4096_all_fit", "quadratic", "n4096_all_fit"),
    Contrast("reset_vs_direct__n65536_all_fit", "reset", "n65536_all_fit"),
    Contrast(
        "reset_vs_direct__n65536_oversized_every64",
        "reset",
        "n65536_oversized_every64",
    ),
    Contrast(
        "reset_vs_direct__n65536_half_oversized_alternating_zero",
        "reset",
        "n65536_half_oversized_alternating_zero",
    ),
    Contrast("prefix_vs_direct__n64_zero_heavy_budget0", "prefix", "n64_zero_heavy_budget0"),
    Contrast(
        "prefix_vs_direct__n4096_uniform_moderate",
        "prefix",
        "n4096_uniform_moderate",
    ),
    Contrast("prefix_vs_direct__n65536_all_fit", "prefix", "n65536_all_fit"),
)
AA_CONTRAST = Contrast(
    "aa_direct__n4096_uniform_moderate",
    "direct",
    "n4096_uniform_moderate",
)
PROFILE_SPECS = (
    ("reference", "n8_zero_heavy_budget0", "count"),
    ("direct", "n65536_all_fit", "count"),
    ("quadratic", "n4096_all_fit", "count"),
    ("reset", "n65536_oversized_every64", "count"),
    ("prefix", "n4096_uniform_moderate", "count"),
)
...
def planned_schedule(phases: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for phase in phases:
        if phase in ("pilot", "main"):
            ...
            for contrast in CONTRASTS:
                for position_index, candidate in enumerate(
                    (BASELINE, contrast.b_candidate), start=1
                ):
                    label = "A" if position_index == 1 else "B"
                    rows.append(
                        asdict(
                            Position(
                                phase,
                                f"quick__{contrast.contrast_id}",
                                1,
                                "AB",
                                position_index,
                                label,
                                ...
                                contrast.operation_phase,
                                stable_seed(
                                    f"fixture:{phase}:{contrast.cell}:"
                                    f"{contrast.operation_phase}:1"
                                ),
                                0,
                                1,
                                DEFAULT_INNER,
...
def build_benchmark(
    ...
    argv = [
        ...
        "bench",
        "--package",
        "topic-005-two-pointers-and-sliding-windows",
        "--bench",
        "bounded_subarrays",
        "--no-run",
        "--locked",
    ...
    for index, line in enumerate(stdout.splitlines(), start=1):
        ...
        if (
            ...
            and target.get("name") == "bounded_subarrays"
    ...
    if (
        ...
        raise ExperimentIncomplete(
            "Cargo did not produce exactly one matching bounded_subarrays benchmark artifact"
        )
    ...
    retained = run_dir / "artifacts" / f"bounded_subarrays-{image_hash}"
...
def validate_record(position: Position, record: Any) -> list[str]:
    ...
    if not isinstance(work, dict):
        ...
        multiplier = position.inner
        expected_exact = {
            "input_values": cell.length * multiplier,
            "mathematical_subarrays": (
                cell.length * (cell.length + 1) // 2 * multiplier
            ),
            "zero_values": cell.zero_values * multiplier,
            "oversized_values": cell.oversized_values * multiplier,
            "candidate_calls": multiplier,
            "candidate_prefix_slots": (
                (cell.length + 1) * multiplier
                if position.candidate == "prefix"
                else 0
            ),
    ...
    expected_canaries = {
        ...
        "observed_count": cell.expected_count,
...
class Executor:
    ...
    def run_block(
        ...
        if phase == "aa":
            if {attempt.candidate for attempt in attempts} != {"direct"}:
                errors.append("A/A did not execute the identical direct artifact")
...
def analyze_main(executor: Executor) -> dict[str, Any]:
    ...
    for contrast in CONTRASTS:
        ...
        row.update(
            {
                "estimand": "candidate/direct geometric mean of complete-block time ratios",
...
def analyze_aa(executor: Executor) -> dict[str, Any]:
    ...
    if {attempt.candidate for attempt in attempts} != {"direct"}:
        mechanical_errors.append("labels did not execute the same direct candidate")
    templates = [
...
def self_check() -> dict[str, Any]:
    errors = []
    if len(CELLS) != 10 or len({cell.name for cell in CELLS}) != 10:
        errors.append("expected ten uniquely named workload cells")
    if len(CONTRASTS) != FAMILY_SIZE:
        errors.append("primary contrast family is not exactly 12")
    expected_matrix = {
        ("reference", "n8_zero_heavy_budget0"),
        ("reference", "n8_all_fit"),
        ("quadratic", "n8_all_fit"),
        ("quadratic", "n64_immediate_reject"),
        ("quadratic", "n4096_immediate_reject"),
        ("quadratic", "n4096_all_fit"),
        ("reset", "n65536_all_fit"),
        ("reset", "n65536_oversized_every64"),
        ("reset", "n65536_half_oversized_alternating_zero"),
        ("prefix", "n64_zero_heavy_budget0"),
        ("prefix", "n4096_uniform_moderate"),
        ("prefix", "n65536_all_fit"),
    }
    if {(item.b_candidate, item.cell) for item in CONTRASTS} != expected_matrix:
        errors.append("the frozen twelve-contrast matrix changed")
    if {contrast.operation_phase for contrast in CONTRASTS} != {"count"}:
        errors.append("primary family operation phase changed")
    expected_profiles = (
        ("reference", "n8_zero_heavy_budget0", "count"),
        ("direct", "n65536_all_fit", "count"),
        ("quadratic", "n4096_all_fit", "count"),
    ...
    if PROFILE_SPECS != expected_profiles:
        errors.append("five representative dynamic profile specifications changed")
    if ASSIGNMENT_SEED != 0x0000_0005_5A17_D0C5:
        errors.append("assignment seed changed")
    if DEFAULT_TIMEOUT_SECONDS != 180.0:
        errors.append("per-process timeout changed from 180 seconds")
    if DEFAULT_INNER != 64:
        errors.append("scheduled inner repetition count changed from 64")
    ...
    identifiers = [attempt_id(Position(**row)) for row in all_schedule]
    if len(identifiers) != len(set(identifiers)):
        errors.append("planned attempt identifiers are not unique")
    quick_schedule = planned_schedule(["quick"])
    if len(quick_schedule) != FAMILY_SIZE * 2:
        errors.append("quick schedule does not cover both arms of every contrast")
